...
- 실제 운영 앱(`qt-trader`, `qt-ops-api`) 재기동 여부는 이 문서 범위 밖이다.
...
이 문서는 운영 DB 반영 자체의 기록이다. 아래는 후속 운영 작업이다.

1. 실제 운영 `qt-trader` / `qt-ops-api`가 새 스키마를 사용하는 코드인지 확인
2. 필요 시 실제 운영 앱 재기동
3. 앱 로그에서 스키마 관련 오류 여부 확인
4. `docs/order_attempts_ops_checks_2026-03-04.md`의 운영 점검 쿼리 재실행
