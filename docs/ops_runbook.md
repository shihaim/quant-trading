...
- 인프라 변경 상세: `docs/handover/infra_handover_2026-02-28.md`
- GitHub-hosted 배포 절차: `docs/runbooks/github_hosted_deployment.md`
- PC 간 PostgreSQL 마이그레이션 런북: `docs/runbooks/cross_pc_postgres_migration_runbook_2026-03-02.md`
- `order_attempts` 운영 DB 반영 기록: `docs/reports/order_attempts_rollout_2026-03-04.md`
...
- 신규 인수인계 문서: `docs/handover/v2_foundation_handover_2026-03-05.md`
- 신규 인증 API:
...
- V3 호환성 fallback 정리 계획: `docs/reports/f2_v3_compatibility_fallback_cleanup_plan_2026-05-16.md`.
...
### 11.1 운영 secret / CORS 기준 (2026-06-01)

Ops API는 production-like runtime에서 안전하지 않은 기본값을 거부한다.

Production-like runtime 기준:

- `TRADE_MODE=REAL`, `TRADE_MODE=TEST`, `TRADE_MODE=SHADOW`
- 또는 `OPS_API_ENV=production`

운영/준운영 배포 전 필수 기준:

- `OPS_API_AUTH_SECRET`는 `dev-ops-auth-secret-change-me`가 아닌 충분히 긴 난수 secret으로 설정한다.
- `OPS_API_CREDENTIALS_ENCRYPTION_KEY`는 `dev-ops-credentials-encryption-key-change-me`가 아닌 운영 key로 설정한다.
- `OPS_API_ALLOW_ORIGIN=*`는 production-like runtime에서 사용할 수 없다. 실제 관리자 웹 origin만 허용한다.
- `OPS_API_CREDENTIALS_ENCRYPTION_KEY`, active key version, keyring 변경은 기존 `user_exchange_credentials` 복호화에 영향을 줄 수 있으므로 rotation runbook 없이 임의 변경하지 않는다.

Preview/local 개발:

- `TRADE_MODE=PAPER`와 `OPS_API_ENV=local` 조합은 기존 개발 기본값과 wildcard CORS를 허용한다.
- 운영 전환 시 `.env.runtime`의 secret과 CORS 값을 먼저 교체한 뒤 Ops API를 시작한다.
